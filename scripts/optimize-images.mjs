...
/* Portret 4:5 — raportul ramei din Hero. Lățimea acoperă ecranele 2x. */
const PROFILE = { width: 760, height: 950, quality: 82 }
...
async function profile() {
  const src = path.join(SRC, 'cristian-gilca.png')
  ...
  await sharp(src)
    ...
    .resize(PROFILE.width, PROFILE.height, { fit: 'cover', position: sharp.strategy.attention })
    .webp({ quality: PROFILE.quality, effort: 6 })
    .toFile(out)

  await report('cristian-gilca.webp', src, out)
}
